...
class WorkoutDiaryDB:
    ...
    def insert_workout_log(self, user_id, exercise_id, start_time, end_time, duration_minutes, notes, favourites):
        query = """
            INSERT INTO workout_Log (user_id, exercise_id, start_time, end_time, duration_minutes, notes, favourites)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
        self.cursor.execute(query, (user_id, exercise_id, start_time, end_time, duration_minutes, notes, favourites))
        self.conn.commit()
    ...
    ''' =========================Ekta Code=========================== '''
    def query_favourite_exercises(self, user_id):
        query = """
            SELECT e.name AS exercise_name, e.muscle
            FROM workout_log wl
            JOIN exercises e ON wl.exercise_id = e.exercise_id
            WHERE wl.user_id = %s AND wl.favourites = TRUE
