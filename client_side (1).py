...
class WorkoutClient:
    ...
    def log_workout(self):
        ...
        '''===================================Ekta code================================'''
        favourites = bool(input("Would you like to add this as a favourite? yes/no:").strip().lower())
        '''==========================================================================='''
    ...
    def view_favourites(self):
        print("=== View Favourite Exercises ===")
        user_id = int(input("User ID: "))
        params = {"user_id": user_id}

        try:
            response = requests.get(f"{BASE_URL}/favourite-exercises", params=params)
            response.raise_for_status()
            data = response.json()

            favourites = data.get("favourites", [])
            if not favourites:
                print("No favourite exercises found.")
                return

            print("Favourite Exercises:")
            for fav in favourites:
                print(f"- {fav['exercise_name']} ({fav['muscle']})")

        except requests.RequestException as e:
            print("Error fetching favourite exercises:", e)        
